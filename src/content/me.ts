...
export const me: MeContent = {
  ...
  stats: [
    ...
    },
    {
      ...
      source: 'HikariCP 10 → 30 pool · 부하 테스트',
    },
  ],
  ...
  stories: [
    {
      ...
      body:
        '“HikariCP 풀을 늘리면 더 빠르다”는 팀의 가설을 그대로 받지 않았다. k6 부하 시나리오를 새로 짜서 처리량과 p99 지연을 함께 측정했고, 오히려 제안된 설정에서 throughput이 17% 떨어지고 p99가 17배 악화된다는 걸 증거로 제시했다. 결과: 튜닝 계획 반려, 기존 설정 유지.',
      bullets: [
        'k6 시나리오를 실사용 트래픽 분포에 맞게 재설계',
        'p50 / p95 / p99 지연과 TPS를 동일 축에서 비교',
        '근거 그래프와 리포트로 의사결정 테이블을 뒤집음',
      ],
    },
    {
      id: 'mongo-migration',
      ...
      body:
        '채팅 데이터 저장소를 PG에서 MongoDB로 옮기면서 듀얼라이트 3단계 전략을 설계했다. 환경변수로 단계 토글, 역방향 롤백 경로 확보, 마지막에 양쪽 카운트를 맞춰 검증. 900,385건 이관이 12분에 끝났고 서비스는 한 번도 멈추지 않았다.',
      bullets: [
        '1단계: 이중 쓰기로 신규 데이터부터 동기화',
        '2단계: 과거 데이터 배치 이관 + 건수·해시 검증',
        '3단계: 읽기 경로 전환, PG를 관측용 섀도로 남김',
      ],
    },
    {
      ...
      body:
        '같은 요청이 브라우저에서 1.7~3.5초, curl에선 38~58ms로 측정되는 모순이 있었다. OpenTelemetry + Tempo로 스팬을 펼쳐보니 connection span이 1.68초, 정작 SQL은 12ms. 경로를 따라 들어가 MongoDB의 `meetingId + createdAt` 복합 인덱스 누락을 찾아냈고, 추가 후 쿼리 시간이 369ms → 16ms로 내려왔다.',
      bullets: [
        '수치 모순을 가설이 아니라 트레이스로 분해',
        'connection span과 실행 시간의 분리를 관측',
        '적중 인덱스 추가 후 전 구간 재측정 검증',
      ],
    },
  ],
